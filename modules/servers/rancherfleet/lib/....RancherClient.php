...
class RancherClient
{
    ...
    // -----------------------------------------------------------------------
    // Phase C: LimitRange (default container limits for quota compliance)
    // -----------------------------------------------------------------------

    /**
     * Applies a LimitRange to a namespace.
     * Sets default CPU and memory limits/requests on containers that do not
     * specify them — required when a ResourceQuota enforces limits.cpu/limits.memory.
     *
     * @param string $namespace
     * @param array  $quota     The quota limits array (cpu, memory keys)
     */
    public function applyLimitRange($namespace, $quota)
    {
        // Convert quota memory to MiB integer for safe arithmetic
        $memLimit    = isset($quota['memory']) ? $quota['memory'] : '512Mi';
        $cpuLimit    = isset($quota['cpu'])    ? $quota['cpu']    : '1';

        $memMib      = $this->toMiB($memLimit);
        $cpuMillis   = $this->toMillicores($cpuLimit);

        // Default container limit = 50% of namespace quota
        $defaultMemMib  = max(128, (int)($memMib / 2));
        $defaultCpuM    = max(100, (int)($cpuMillis / 2));

        // Request = 50% of the default limit (25% of quota)
        // Always strictly LESS than the default limit
        $requestMemMib  = max(64, (int)($defaultMemMib / 2));
        $requestCpuM    = max(50, (int)($defaultCpuM / 2));

        // Format back to Kubernetes quantities
        $defaultMem  = $defaultMemMib  . 'Mi';
        $requestMem  = $requestMemMib  . 'Mi';
        $defaultCpu  = $defaultCpuM    . 'm';
        $requestCpu  = $requestCpuM    . 'm';

        $body = array(
            'apiVersion' => 'v1',
            'kind'       => 'LimitRange',
            'metadata'   => array(
                'name'      => 'rancherfleet-limits',
                'namespace' => $namespace,
            ...
            'spec' => array(
                'limits' => array(
                    array(
                        'type'           => 'Container',
                        'default'        => array('cpu' => $defaultCpu, 'memory' => $defaultMem),
                        'defaultRequest' => array('cpu' => $requestCpu, 'memory' => $requestMem),
                    ),
                ),
            ),
        );

        Logger::info("applyLimitRange: {$namespace} defaultCpu={$defaultCpu} defaultMem={$defaultMem} requestCpu={$requestCpu} requestMem={$requestMem}");

        // Delete existing LimitRange first to avoid stale invalid values
        $url = $this->k8sUrl('/api/v1/namespaces/' . $namespace . '/limitranges/rancherfleet-limits');
        try {
            $this->request('DELETE', $url);
        } catch (RancherApiException $e) {
            // 404 is fine — didn't exist
        }

        // Create fresh
        $this->request('POST', $this->k8sUrl('/api/v1/namespaces/' . $namespace . '/limitranges'), $body);
    }

    /**
     * Converts a Kubernetes memory quantity to MiB integer.
     * e.g. "2Gi" -> 2048, "512Mi" -> 512, "1Gi" -> 1024
     */
    private function toMiB($quantity)
    {
        $quantity = trim($quantity);
        if (substr($quantity, -2) === 'Gi') return (int)((float)rtrim($quantity, 'Gi') * 1024);
        if (substr($quantity, -2) === 'Mi') return (int)rtrim($quantity, 'Mi');
        if (substr($quantity, -2) === 'Ki') return (int)(rtrim($quantity, 'Ki') / 1024);
        if (substr($quantity, -1) === 'G')  return (int)((float)rtrim($quantity, 'G') * 953);
        if (substr($quantity, -1) === 'M')  return (int)((float)rtrim($quantity, 'M') * 0.953);
        return (int)$quantity;
    }

    /**
     * Converts a Kubernetes CPU quantity to millicores integer.
     * e.g. "2" -> 2000, "500m" -> 500, "0.5" -> 500
     */
    private function toMillicores($quantity)
    {
        $quantity = trim($quantity);
        if (substr($quantity, -1) === 'm') return (int)rtrim($quantity, 'm');
        if (substr($quantity, -1) === 'n') return (int)(rtrim($quantity, 'n') / 1000000);
    ...
    public function applyResourceQuota($namespace, $limits)
    {
        ...
        // Only enforce storage, pods and services — not CPU/memory limits.
        // Enforcing limits.cpu/limits.memory requires every container to declare
        // resource limits which conflicts with Odoo's default manifests.
        $body = array(
            'apiVersion' => 'v1',
            'kind'       => 'ResourceQuota',
            'metadata'   => array(
...
";
    ...
    {
        // Approach 1: Rancher Steve API (/v1/) — handles Accept headers correctly
        $steveParams = 'tailLines=' . (int)$tailLines . '&timestamps=true&sinceSeconds=' . (int)$sinceSeconds;
        if ($containerName) {
            $steveParams .= '&container=' . urlencode($containerName);
        }
        $steveUrl = $this->baseUrl . '/v1/pods/' . $namespace . '/' . $podName . '/log?' . $steveParams;

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,            $steveUrl);
        ...
        if (!$curlErr && $httpCode >= 200 && $httpCode < 300 && !empty(trim($raw))) {
            return $raw;
        }

        // Approach 2: kubectl-proxy with application/json Accept header
        $k8sParams = 'sinceSeconds=' . (int)$sinceSeconds . '&tailLines=' . (int)$tailLines . '&timestamps=true';
        if ($containerName) {
            $k8sParams .= '&container=' . urlencode($containerName);
        }
        $k8sUrl = $this->k8sUrl('/api/v1/namespaces/' . $namespace . '/pods/' . $podName . '/log?' . $k8sParams);

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,            $k8sUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_TIMEOUT,        30);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        ...
        if ($httpCode2 >= 200 && $httpCode2 < 300 && !empty(trim($raw2))) {
        ...
        throw new RancherApiException(
            'Log fetch error [' . $httpCode . ']: Rancher proxy rejected log request. Verify API token has log access.',
            $httpCode,
            $raw
        );
    }
