...
function rancherfleet_ConfigOptions()
{
    return array(
        ...
        'CPU Limit' => array(
            'FriendlyName' => 'CPU Limit (ResourceQuota)',
            'Type'         => 'text',
            'Size'         => '10',
            'Default'      => '2',
            'Description'  => 'Max CPU cores per namespace e.g. 2 or 500m. Leave blank to skip quota.',
        ),
        'Memory Limit' => array(
            'FriendlyName' => 'Memory Limit (ResourceQuota)',
            'Type'         => 'text',
            'Size'         => '10',
            'Default'      => '2Gi',
            'Description'  => 'Max memory per namespace e.g. 2Gi or 512Mi. Leave blank to skip quota.',
        ),
        'Storage Limit' => array(
            'FriendlyName' => 'Storage Limit (ResourceQuota)',
            'Type'         => 'text',
            'Size'         => '10',
            'Default'      => '10Gi',
            'Description'  => 'Max persistent storage per namespace e.g. 10Gi. Leave blank to skip.',
        ),
        'Pod Limit' => array(
            'FriendlyName' => 'Pod Limit (ResourceQuota)',
            'Type'         => 'text',
            'Size'         => '10',
            'Default'      => '20',
            'Description'  => 'Max number of pods per namespace. Leave blank to skip.',
        ),
        'Services Limit' => array(
            'FriendlyName' => 'Services Limit (ResourceQuota)',
            'Type'         => 'text',
            'Size'         => '10',
            'Default'      => '10',
            'Description'  => 'Max number of services per namespace. Leave blank to skip.',
        ),
        ...
        'Container CPU Request' => array(
            ...
            'Size'         => '10',
            'Default'      => '',
            'Description'  => 'CPU request injected into deployment manifests e.g. 250m. Leave blank to use template values.',
        ),
        'Container Memory Request' => array(
            ...
            'Size'         => '10',
            'Default'      => '',
            'Description'  => 'Memory request injected into deployment manifests e.g. 512Mi. Leave blank to use template values.',
        ),
        'Custom Image' => array(
        ...
        'User Count' => array(
            ...
            'Default'      => '5',
            'Description'  => 'Number of users this instance will support. Determines resource quota limits applied to the namespace.',
        ),
    );
}
...
{
    $table = array(
        2  => array('cpu' => '1',  'memory' => '1Gi',  'storage' => '5Gi',   'pods' => '10', 'services' => '5'),
        5  => array('cpu' => '2',  'memory' => '2Gi',  'storage' => '10Gi',  'pods' => '15', 'services' => '8'),
        15 => array('cpu' => '3',  'memory' => '4Gi',  'storage' => '20Gi',  'pods' => '20', 'services' => '10'),
        20 => array('cpu' => '4',  'memory' => '6Gi',  'storage' => '30Gi',  'pods' => '25', 'services' => '10'),
        35 => array('cpu' => '6',  'memory' => '8Gi',  'storage' => '40Gi',  'pods' => '30', 'services' => '12'),
        45 => array('cpu' => '8',  'memory' => '10Gi', 'storage' => '50Gi',  'pods' => '35', 'services' => '15'),
        55 => array('cpu' => '10', 'memory' => '12Gi', 'storage' => '60Gi',  'pods' => '40', 'services' => '15'),
        65 => array('cpu' => '12', 'memory' => '16Gi', 'storage' => '80Gi',  'pods' => '45', 'services' => '20'),
        75 => array('cpu' => '14', 'memory' => '20Gi', 'storage' => '100Gi', 'pods' => '50', 'services' => '20'),
        85 => array('cpu' => '16', 'memory' => '24Gi', 'storage' => '120Gi', 'pods' => '55', 'services' => '25'),
        95 => array('cpu' => '20', 'memory' => '32Gi', 'storage' => '150Gi', 'pods' => '60', 'services' => '25'),
    );
...
/**
 * Returns quota limits from product config options.
 * If manual CPU/Memory/Storage limits are blank, derives them from User Count.
 */
function rancherfleet_getQuotaLimits(array $params)
{
    $co = isset($params['configoptions']) ? $params['configoptions'] : array();

    $manualCpu     = !empty($co['CPU Limit'])      ? trim($co['CPU Limit'])      : (isset($params['configoption2']) ? trim($params['configoption2']) : '');
    $manualMemory  = !empty($co['Memory Limit'])   ? trim($co['Memory Limit'])   : (isset($params['configoption3']) ? trim($params['configoption3']) : '');
    $manualStorage = !empty($co['Storage Limit'])  ? trim($co['Storage Limit'])  : (isset($params['configoption4']) ? trim($params['configoption4']) : '');
    $manualPods    = !empty($co['Pod Limit'])       ? trim($co['Pod Limit'])      : (isset($params['configoption5']) ? trim($params['configoption5']) : '');
    $manualSvcs    = !empty($co['Services Limit'])  ? trim($co['Services Limit']) : (isset($params['configoption6']) ? trim($params['configoption6']) : '');

    // If any manual limit is set, use manual values
    if ($manualCpu || $manualMemory || $manualStorage) {
    ...
    // Fall back to user-count derived quotas
    $userCount = rancherfleet_getUserCount($params);
    $quota     = rancherfleet_getUserQuota($userCount);
    RancherFleet\Logger::info("getQuotaLimits: using user-count quota for {$userCount} users: " . json_encode($quota));
    return $quota;
}

/**
 * Returns the Odoo image version from the service custom fields.
 * Falls back to '19' if not set.
 */
function rancherfleet_getOdooImageVersion(array $params)
{
    ...
    try {
        $field = \WHMCS\Database\Capsule::table('tblcustomfieldsvalues')
            ...
            ->where('tblcustomfields.fieldname', 'Odoo Version')
            ->value('tblcustomfieldsvalues.value');
        if (!empty($field)) return trim($field);
    } catch (\Exception $e) { /* fallback */ }

    // Also check params directly
    foreach (array('customfields', 'configoptions') as $key) {
        if (isset($params[$key]['Odoo Version']) && !empty($params[$key]['Odoo Version'])) {
            return trim($params[$key]['Odoo Version']);
        }
    }
    return '19';
}
...
/**
 * Returns the database server identifier from the service custom fields.
 * Falls back to 'db16' if not set.
 */
function rancherfleet_getDbServer(array $params)
{
    ...
    try {
        $field = \WHMCS\Database\Capsule::table('tblcustomfieldsvalues')
            ->join('tblcustomfields', 'tblcustomfields.id', '=', 'tblcustomfieldsvalues.fieldid')
            ->where('tblcustomfieldsvalues.relid', $serviceId)
            ->where('tblcustomfields.fieldname', 'Database Server')
            ->value('tblcustomfieldsvalues.value');
        if (!empty($field)) return trim($field);
    } catch (\Exception $e) { /* fallback */ }
...
/**
 * Returns per-client container resource limits from product config.
...
function rancherfleet_getContainerLimits(array $params)
{
    $co = isset($params['configoptions']) ? $params['configoptions'] : array();
    return array_filter(array(
        'cpu'    => !empty($co['Container CPU Request'])    ? trim($co['Container CPU Request'])    : (isset($params['configoption8']) ? trim($params['configoption8']) : ''),
        'memory' => !empty($co['Container Memory Request']) ? trim($co['Container Memory Request']) : (isset($params['configoption9']) ? trim($params['configoption9']) : ''),
    ));
}
...
function rancherfleet_buildClients(array $params)
{
    ...
    // target_cluster_id = downstream cluster (where namespaces and deployments run)
...
function rancherfleet_CreateAccount(array $params)
{
    ...
    if (rancherfleet_isDryRun($params)) {
        ...
        $quotaLimits = rancherfleet_getQuotaLimits($params);
        ...
        RancherFleet\Logger::info("DryRun: quota=" . json_encode($quotaLimits));
    ...
    try {
        ...
        // Phase C: Apply resource quota if configured
        $quotaLimits = rancherfleet_getQuotaLimits($params);
        if (!empty($quotaLimits)) {
            $rancher->applyResourceQuota($namespace, $quotaLimits);
            RancherFleet\Logger::info("CreateAccount: quota applied to {$namespace}");
        }
...
function rancherfleet_TerminateAccount(array $params)
{
    try {
        ...
        // Allow 2 seconds for Kubernetes to process the namespace deletion
        sleep(2);
...
function rancherfleet_injectResourceLimits($content, $limits)
{
    ...
    $cpu    = isset($limits['cpu'])    ? $limits['cpu']    : null;
    $memory = isset($limits['memory']) ? $limits['memory'] : null;
    ...
    // Build the resources block to inject
    $resourcesBlock  = "          resources:\n";
    $resourcesBlock .= "            requests:\n";
    if ($cpu)    $resourcesBlock .= "              cpu: '" . $cpu . "'\n";
    if ($memory) $resourcesBlock .= "              memory: '" . $memory . "'\n";
    $resourcesBlock .= "            limits:\n";
    if ($cpu)    $resourcesBlock .= "              cpu: '" . $cpu . "'\n";
    if ($memory) $resourcesBlock .= "              memory: '" . $memory . "'\n";


    // Replace existing resources: block under containers
    if (preg_match('/^(\s+resources\s*:.*?)(?=^\s+\w|\Z)/ms', $content)) {
        ...
        // Inject after the image: line in the content
        $content = preg_replace(
            '/(^\s+image\s*:.*$)/m',
            '$1\n' . $resourcesBlock,
            $content,
...
function rancherfleet_DryRunProvision(array $params)
{
    try {
        ...
        try {
            ...
            if (!empty($quotaLimits)) {
                $report[] = "  + ResourceQuota: " . json_encode($quotaLimits);
            }
        } catch (Exception $e) {
...
function rancherfleet_ApplyQuota(array $params)
{
    try {
        ...
        $quotaLimits = rancherfleet_getQuotaLimits($params);
        if (empty($quotaLimits)) {
            return 'No quota limits configured. Set CPU/Memory/Storage/Pod limits in the product Module Settings.';
        }

        $rancher->applyResourceQuota($namespace, $quotaLimits);
        rancherfleet_logHistory($params, 'Quota Applied', json_encode($quotaLimits));
        RancherFleet\Logger::info("ApplyQuota: SUCCESS for {$namespace}");

        return "Success: ResourceQuota applied to '{$namespace}' - " . implode(', ', array_map(
            function($k, $v) { return "{$k}={$v}"; },
            array_keys($quotaLimits),
            array_values($quotaLimits)
        ));
    } catch (Exception $e) {
        $detail = rancherfleet_exceptionDetail($e);
...
/**
 * Generates the full HTML for the client area panel.
 */
