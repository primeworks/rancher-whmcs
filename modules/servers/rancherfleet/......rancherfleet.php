...
require_once __DIR__ . '/lib/Domains/ResellerClubClient.php';
...
function rancherfleet_ConfigOptions()
{
    return array(
        ...
        'ResellerClub Reseller ID' => array(
            'FriendlyName' => 'ResellerClub Reseller ID',
            'Type'         => 'text',
            'Size'         => '20',
            'Default'      => '',
            'Description'  => 'Your ResellerClub auth-userid, from Reseller Control Panel > Manage Profile.',
        ),
        'ResellerClub API Key' => array(
            'FriendlyName' => 'ResellerClub API Key',
            'Type'         => 'password',
            'Size'         => '40',
            'Default'      => '',
            'Description'  => 'Your ResellerClub api-key, from Reseller Control Panel > Settings > API.',
        ),
        'ResellerClub Sandbox Mode' => array(
            'FriendlyName' => 'ResellerClub Sandbox Mode',
            'Type'         => 'yesno',
            'Default'      => 'no',
            'Description'  => 'When enabled, uses test.httpapi.com (sandbox) instead of httpapi.com (live). Use a Demo Account Reseller ID/API Key when enabled.',
        ),
        ...
        'Domain Retry Interval Hours' => array(
            ...
            'Size'         => '5',
            'Default'      => '2',
            'Description'  => 'How often to retry ResellerClub registration after a failure (e.g. underfunded reseller wallet) before giving up and refunding.',
        ),
...
/**
 * Builds the ResellerClub client and DomainOrderManager from this
 * product's ConfigOptions. Separate from rancherfleet_buildClients()
 * since domain purchasing is independent of GitHub/Fleet provisioning —
 * a product could have one configured without the other.
 *
 * @param  array $params
 * @return array  array($resellerClubClient, $domainOrderManager, $rancherClient)
 * @throws \Exception  if ResellerClub credentials are not configured
 */
function rancherfleet_buildDomainClients(array $params)
{
    $co = isset($params['configoptions']) ? $params['configoptions'] : array();

    $resellerId = !empty($co['ResellerClub Reseller ID']) ? trim($co['ResellerClub Reseller ID'])
                : (isset($params['configoption15']) ? trim($params['configoption15']) : '');
    $apiKey     = !empty($co['ResellerClub API Key']) ? trim($co['ResellerClub API Key'])
                : (isset($params['configoption16']) ? trim($params['configoption16']) : '');
    $sandbox    = !empty($co['ResellerClub Sandbox Mode']) && strtolower($co['ResellerClub Sandbox Mode']) === 'on';

    if (empty($resellerId) || empty($apiKey)) {
        throw new \Exception('ResellerClub Reseller ID and API Key must be configured in Module Settings before domains can be purchased.');
    }

    $baseUrl = $sandbox ? 'https://test.httpapi.com' : 'https://httpapi.com';
    $rc      = new RancherFleet\Domains\ResellerClubClient($baseUrl, $resellerId, $apiKey);
    ...
    $orderManager = new RancherFleet\Domains\DomainOrderManager($rc, $rancher);

    return array($rc, $orderManager, $rancher);
}
...
/**
 * Handles a domain purchase POST from the client area. Collects the
 * registrant form fields, runs the full DomainOrderManager::placeOrder()
 * flow, and returns a message string consumed by rancherfleet_clientAreaHtml().
 *
 * @param  array  $params
...
{
    $domainName = isset($_POST['domain_name']) ? trim($_POST['domain_name']) : '';
    if (!$domainName) {
        return 'domain_error:No domain name provided.';
    }

    $required = array('reg_name', 'reg_email', 'reg_address', 'reg_city', 'reg_state', 'reg_country', 'reg_zip', 'reg_phone_cc', 'reg_phone');
    ...
    try {
        list($rc, $orderManager) = rancherfleet_buildDomainClients($params);

        $clientId = (int)$params['userid'];

        $contactDetails = array(
            'name'            => $_POST['reg_name'],
            'company'         => isset($_POST['reg_company']) ? $_POST['reg_company'] : $_POST['reg_name'],
            'email'           => $_POST['reg_email'],
            'address-line-1'  => $_POST['reg_address'],
            'city'            => $_POST['reg_city'],
            'state'           => $_POST['reg_state'],
            'country'         => $_POST['reg_country'],
            'zipcode'         => $_POST['reg_zip'],
            'phone-cc'        => $_POST['reg_phone_cc'],
            'phone'           => $_POST['reg_phone'],
        );

        // Find or create the ResellerClub customer entity for this WHMCS client.
        $customerId = $rc->findCustomerByUsername($params['clientsdetails']['email']);
        if (!$customerId) {
            $customerId = $rc->createCustomer(array_merge($contactDetails, array(
                'username' => $params['clientsdetails']['email'],
                'passwd'   => bin2hex(random_bytes(12)) . 'Aa1!', // not used for login by the client; ResellerClub requires a password on the entity
            )));
        }

        $contactId = $rc->createContact(array_merge($contactDetails, array(
            'customer-id' => $customerId,
        )));

        $pricing = $rc->getCustomerPricing();
        $tld     = substr($domainName, strrpos($domainName, '.') + 1);
        $price   = $rc->extractYearOnePrice($pricing, $tld);
        if ($price === null) {
        ...
        // Look up the client's actual Odoo Service name in this namespace —
        // needed as a valid (if unused, due to the redirect annotation)
        // backend reference for the root-redirect Ingress.
        list($rancher) = rancherfleet_buildClients($params);
        ...
        $orderData = array(
            ...
            'years'              => 1,
            'ns'                 => RancherFleet\Domains\ResellerClubClient::DEFAULT_NAMESERVERS,
...
/**
 * Renders the domain search/purchase panel for the client area.
 *
 * Search itself happens via a simple same-page POST (domain_search action)
 * rather than a JS/AJAX call, consistent with how the Restart button on
 * this same panel already works — keeps the panel functional without
 * depending on additional JS wiring being added to clientarea.tpl.
 */
function rancherfleet_domainPanelHtml(array $params, $namespace, $orderNum)
{
    $html = '<div class="rfm-ca-card">';
    $html .= '<h4>Get a Domain Name</h4>';

    $co = isset($params['configoptions']) ? $params['configoptions'] : array();
    $resellerConfigured = !empty($co['ResellerClub Reseller ID']) || !empty($params['configoption15']);
    ...
    $html .= '<p style="font-size:12px;color:#666;margin-bottom:10px;">Search for a domain name. Once purchased, <code>www.yourdomain.com</code> will automatically be connected to this instance, and visitors to the root domain will be redirected to it.</p>';
    ...
    if ($searchAction === 'domain_search' && $searchTerm) {
        try {
            list($rc) = rancherfleet_buildDomainClients($params);

            $tlds    = rancherfleet_commonTlds();
            $results = $rc->checkAvailability($searchTerm, $tlds);
            $pricing = $rc->getCustomerPricing();

            $html .= '<div style="margin-top:8px;">';
            foreach ($results as $fullDomain => $info) {
                ...
                $html .= '<span style="font-family:monospace;">' . htmlspecialchars($fullDomain) . '</span>';
                ...
                if ($available) {
                    ...
                    $html .= '<button type="button" onclick="document.getElementById(\'rfm-domain-modal-' . md5($fullDomain) . '\').style.display=\'block\'" style="background:#27ae60;color:#fff;border:none;border-radius:4px;padding:6px 14px;font-size:12px;font-weight:bold;cursor:pointer;">Buy</button>';
                    $html .= '</span>';
                } else {
                ...
                if ($available) {
                    $html .= rancherfleet_domainPurchaseModal($fullDomain, $serviceUrl);
                }
            }
            $html .= '</div>';
...
/**
 * Renders a simple modal (no JS framework dependency — plain CSS display
 * toggle) containing the registrant contact form for a specific domain.
 */
function rancherfleet_domainPurchaseModal($domainName, $serviceUrl)
{
    ...
    $html .= '<input type="hidden" name="domain_name" value="' . htmlspecialchars($domainName) . '">';
    ...
    $fields = array(
        ...
        'reg_phone_cc' => 'Phone Country Code (e.g. 1)',
    ...
    $html .= '<p style="font-size:11px;color:#888;margin-top:12px;">This information is required by ICANN for domain registration and will be used as the public WHOIS registrant contact unless privacy protection is enabled separately.</p>';
