...
// ---------------------------------------------------------------------------
// Phase C: Resource Quota config helper
// ---------------------------------------------------------------------------

/**
 * Returns quota limits from product config options.
 * Returns empty array if all limits are blank (no quota applied).
 */
function rancherfleet_getQuotaLimits(array $params)
{
    ...
    $limits = array_filter(array(
        'cpu'      => !empty($co['CPU Limit'])       ? trim($co['CPU Limit'])       : (isset($params['configoption2']) ? trim($params['configoption2']) : ''),
        'memory'   => !empty($co['Memory Limit'])    ? trim($co['Memory Limit'])    : (isset($params['configoption3']) ? trim($params['configoption3']) : ''),
        'storage'  => !empty($co['Storage Limit'])   ? trim($co['Storage Limit'])   : (isset($params['configoption4']) ? trim($params['configoption4']) : ''),
        'pods'     => !empty($co['Pod Limit'])        ? trim($co['Pod Limit'])       : (isset($params['configoption5']) ? trim($params['configoption5']) : ''),
        'services' => !empty($co['Services Limit'])  ? trim($co['Services Limit'])  : (isset($params['configoption6']) ? trim($params['configoption6']) : ''),
    ));
    return $limits;
}
...
function rancherfleet_CreateAccount(array $params)
{
    ...
    try {
        list($rancher, $github) = rancherfleet_buildClients($params);
        // Phase D: pass per-client container limits and Phase E custom image
        $containerLimits = rancherfleet_getContainerLimits($params);
        $customImage     = rancherfleet_getCustomImage($params);
        $github->bootstrapClientFolder($repoPath, $namespace, $orderNum, $containerLimits, $customImage);
